...
export const viewport = { themeColor: "#004a8e", width: "device-width", initialScale: 1 };

export async function generateMetadata({ params }: { params: Promise<{ category: string, slug: string }> }): Promise<Metadata> {
  const { category, slug } = await params;
  const post = await getSinglePost(slug);
  if (!post) return { title: "Not Found" };
  return { title: post.title, openGraph: { images: [post.image || "/logo-md.png"] } };
}

export default async function PostDetail({ params }: { params: Promise<{ category: string, slug: string }> }) {
  const { category, slug } = await params;
  const [post, relatedPosts] = await Promise.all([ getSinglePost(slug), getRelatedPosts(category, slug) ]);
  ...
  return (
    <main className="post-detail-main">
            ...
            <section className="related-box">
              ...
              <div className="related-grid">
                {relatedPosts.slice(0, 3).map((rel: any) => (
                  <Link href={`/${category}/${rel.slug.current}`} key={rel._id} className="related-item">
        ...
        <aside className="sidebar">
          ...
          <div className="sidebar-widget">
            ...
            <div className="social-grid">
              <a href="https://facebook.com/pcmkembaran" target="_blank" className="sc-icon fb">
                <svg width="24" height="24" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </a>
              <a href="https://instagram.com/pcmkembaran" target="_blank" className="sc-icon ig">
                <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>
              </a>
              <a href="https://wa.me/6281234567890" target="_blank" className="sc-icon wa">
          ...
          <div className="sidebar-widget">
            ...
            <div className="popular-list">
              {relatedPosts?.slice(0, 5).map((pop: any, idx: number) => (
                <Link href={`/${category}/${pop.slug.current}`} key={pop._id} className="pop-item">
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .main-title { font-size: 38px; font-weight: 900; line-height: 1.2; color: #111; margin-bottom: 20px; }
        ...
        .related-item h4 { font-size: 15px; font-weight: 700; line-height: 1.4; }
        ...
        .pop-text { font-size: 14px; font-weight: 700; line-height: 1.4; color: #333; transition: 0.2s; }
